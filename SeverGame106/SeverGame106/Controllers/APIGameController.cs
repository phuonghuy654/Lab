...
using ServerGame106.Views.ViewModel;
...
namespace ServerGame106.Controllers
{
    ...
    public class APIGameController : ControllerBase
    {
        ...
        public IActionResult Get()
        {
            ...
            {
                CourseName = "Hello, Back-End Game Programming",
                CourseCode = "GAM106",
                Name = "Tran Duc Hoang",
                StudentCode = "PK04069",
                Class = "GA19305",
        ...
        [HttpGet("GetUserInfoInformation/{userId}")]
        public async Task<IActionResult> GetUserInfoInformation(string userId)
        {
            try
            {
                ...
                UserInformationVM userInfoInformationVM = new();
                userInfoInformationVM.Name = user.Name;
                userInfoInformationVM.Email = user.Email;
                userInfoInformationVM.avatar = user.Avatar;
                userInfoInformationVM.Region = await _db.Regions.Where(x => x.RegionId == user.RegionId)
                    .Select(x => x.Name).FirstOrDefaultAsync();

                _response.IsSuccess = true;
                _response.Notification = "Lấy dữ liệu thành công";
                _response.Data = userInfoInformationVM;
                return Ok(_response);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordDTO resetPasswordDTO)
        {
            try
            {
                ...
                if (user.OTP == stringOTP)
                {
                    ...
                    user.OTP = $"{stringOTP}_user_" + now.ToString("yyyy_MM_dd_HH_mm_ss");
